...
class TestPerft:
    ...
    def _perft(self, board, depth):
        if depth == 0:
            return 1
        return sum(
            self._perft(board.make_move(move), depth - 1)
            for move in self.gen.generate_legal_moves(board)
        )

    def test_starting_position_depth_three(self):
        board = Board.from_fen()
    ...
    def test_kiwipete_depth_two(self):
        board = Board.from_fen(
            "r3k2r/p1ppqpb1/bn2pnp1/3PN3/"
            "1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1"
        )

        assert self._perft(board, 1) == 48
        assert self._perft(board, 2) == 2039
